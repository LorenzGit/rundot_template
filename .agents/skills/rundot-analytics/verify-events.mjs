...
if (!gameDir) { console.error('usage: verify-events.mjs <gameDir> <port> [--seconds=25]'); process.exit(1); }
...
for (let i = 0; i < 60 && !up; i += 1) {
  try { const r = await fetch(base); up = r.ok; } catch { await new Promise((r) => setTimeout(r, 500)); }
}
if (!up) { console.error(JSON.stringify({ game: path.basename(gameDir), error: 'dev server never came up' })); stop(); process.exit(1); }
