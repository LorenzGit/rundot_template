---
name: rundot-feature-analytics
description: Add analytics to a RUN game — a fire-and-forget event and funnel core, standard economy helpers, and a wiring guide for hooking other game systems' events into it. Ships copy-in TypeScript templates extracted from a shipped RUN game plus an AI-first integration guide. Use when a creator wants event tracking, funnels, telemetry, or to measure where players drop off.
---

# Add analytics to a RUN game

Copy-in implementation extracted from a shipped RUN game. The authoritative
integration guide is `systems/analytics/README.md` — read it top to bottom
before writing code. It derives every game-specific decision (which events and
funnels exist, where they fire) from the host game's own code, so you should
...
| Path | What |
|---|---|
| `systems/analytics/README.md` | Full integration guide: wiring steps, config reference, derive-from-host table, verification checklist |
| `systems/analytics/analytics.ts` | The template you copy into the host game: the fire-and-forget event/funnel core and economy helpers |
...
1. Read `systems/analytics/README.md` fully, then `references/run-sdk-notes.md`.
...
3. Copy the template into the host (suggested home: `src/helpers/analytics/`),
...
5. Wire per the README's numbered steps, then run its verification checklist —
   actually confirm events emit on a real run, don't just confirm it compiles.
...
Every other `rundot-feature-*` system exposes hooks that wire into this (see
the README's Wiring section).
