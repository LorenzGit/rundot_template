...
import { mkdirSync, readdirSync, rmSync, writeFileSync, existsSync, copyFileSync } from "node:fs";
import { dirname, join, resolve } from "node:path";
import { fileURLToPath } from "node:url";

const __dirname = dirname(fileURLToPath(import.meta.url));
const CUTOUT = resolve(__dirname, "../../bg-removal-softshadows/cutout.mjs");
...
if (!input || !outDir) {
    console.error("Usage: node matte_with_softshadows.mjs --in clip.mp4 --out-dir ./frames [options]");
    process.exit(1);
}
...
run("ffmpeg", ["-y", "-i", resolve(input), "-vf", vf, "-start_number", "0", join(raw, "f%05d.png")]);
...
const key = keyArg === "auto" ? sampleKey(join(raw, names[0])) : keyArg.replace(/^#/, "");
console.error(
    JSON.stringify({
        step: "matte",
...
for (const name of names) {
    ...
    if (maxHAfter > 0) {
        run("ffmpeg", ["-y", "-i", dstFull, "-vf", `scale=-2:'min(ih,${maxHAfter})'`, dstFull]);
    }
