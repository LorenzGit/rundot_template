...
import { parseHex, chromaMatte, chromaDecontamMatte, studioWhiteMatte, temporalMedianAlpha } from "./matte_frame.mjs";
...
async function main() {
    ...
    const work = keepWork ? resolve(keepWork) : join(resolve(dirname(output)), `.vbg-${Date.now()}`);
    ...
    const scaleFilter = maxHeight > 0 ? `fps=${fps},scale=-2:'min(ih,${maxHeight})':${SWS}` : `fps=${fps}`;

    console.log(
        JSON.stringify({ step: "extract", fps, engine, input: inAbs, scale: maxHeight > 0 ? "lanczos" : "none" }),
    );
    run("ffmpeg", ["-y", "-i", inAbs, "-vf", scaleFilter, "-start_number", "0", join(framesIn, "f%05d.png")]);
    ...
    // Probe WebM alpha
    const probe = spawnSync("ffprobe", ["-v", "error", "-show_streams", outAbs], { encoding: "utf8" });
    const hasAlpha = (probe.stdout || "").includes("alpha_mode") || (probe.stdout || "").includes("yuva");
    ...
    if (alsoHevc) {
        ...
        try {
            ...
            const hevcProbe = spawnSync("ffprobe", ["-v", "error", "-show_streams", hevcOut], { encoding: "utf8" });
            hevcOk = (hevcProbe.stdout || "").includes("hevc") || (hevcProbe.stdout || "").includes("hvc1");
        } catch (err) {
            console.warn(
                JSON.stringify({
...
async function processWithSharp(io, names, framesIn, framesOut, engine, opt) {
    ...
    for (const name of names) {
        ...
        else if (engine === "studio-white") out = studioWhiteMatte(rgba, w, h, opt.whiteThreshold, 28, opt.softness);
...
async function processWithPillow(names, framesIn, framesOut, engine, opt) {
    ...
    if (
        ...
        const softshadowsCutout = resolve(__dirname, "../../bg-removal-softshadows/cutout.mjs");
        let decodePng;
        let encodePng;
        ...
        if (decodePng && encodePng) {
            ...
            const final = opt.temporalR > 0 ? temporalMedianAlpha(matted, opt.temporalR) : matted;
            for (let i = 0; i < names.length; i++) {
                writeFileSync(join(framesOut, names[i]), encodePng({ w, h, data: final[i] }));
            }
            return;
        }
    }
...
async function runBirefnet(names, framesIn, framesOut, opt) {
    const pyScript = opt.birefnetPy || resolve(__dirname, "../../bg-removal-softshadows/birefnet_cutout.py");
    const python = opt.birefnetPython || resolve(__dirname, "../../bg-removal-softshadows/.venv/bin/python");
